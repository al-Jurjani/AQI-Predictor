...
def handle_missing_values(df):
    df.replace([np.inf, -np.inf], np.nan, inplace=True) # replaces infinite or invalid numbers
    df.dropna(axis = 1, how = 'all', inplace = True) # drops completely empty columns (rare)

    numeric_cols = df.select_dtypes(include=[np.number]).columns
    df[numeric_cols] = df[numeric_cols].fillna(df[numeric_cols].mean()) # filling missing numeric values with the column mean
